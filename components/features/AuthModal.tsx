...
const AuthModal: React.FC<AuthModalProps> = ({ onAuthSuccess, onCancel, defaultMode = 'signup' }) => {
  ...
  useEffect(() => {
    // Check if user has already built a persona as a guest
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        // Handle case where email confirmation is required (default in Supabase)
        ...
        // Success
        onAuthSuccess(!isLogin); // Pass true if signup, false if login
      }
    } catch (e: any) {
